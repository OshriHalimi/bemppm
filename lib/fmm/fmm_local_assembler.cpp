...
FmmLocalAssembler<BasisFunctionType, ResultType>::FmmLocalAssembler(
    const Bempp::Space<BasisFunctionType>& space, 
    const Bempp::AssemblyOptions& options, bool conjugateBasis)
  : m_conjugateBasis(conjugateBasis)
{
  ...
  typedef Bempp::LocalAssemblerConstructionHelper Helper;

  // Collect grid data, formerly this worked on the dualToRange space = test
  Helper::collectGridData(space, m_rawGeometry, m_geometryFactory);
  ...
  std::vector<Vector<ResultType> > & result)
{
  ...
  for (int testIndex = 0; testIndex < elementCount; ++testIndex){
    ...
    quadVariants[testIndex] = QuadVariant(integrator,
