    ...
    unsigned int dofStartTrial, unsigned int dofCountTrial) const
{
  ...
  shared_ptr<const Bempp::LocalDofLists<BasisFunctionType>>
      testDofLists  = m_testDofListsCache->get(dofStartTest, dofCountTest);
  shared_ptr<const Bempp::LocalDofLists<BasisFunctionType>>
      trialDofLists = m_trialDofListsCache->get(dofStartTrial, dofCountTrial);
  // Necessary elements
  ...
  */
  ...
  for(size_t nTerm = 0; nTerm < m_assemblers.size(); ++nTerm) {
    m_assemblers[nTerm]->evaluateLocalWeakForms(testElementIndices,
                                                trialElementIndices,
                                                localResult);
...
{
  OctreeNode<ResultType> &node = octree->getNode(n, octree->levels());
  ...
  if (node.trialDofCount()) {
    ...
    nearFieldMats[0] = evaluateNearFieldIntegrals(testDofStart, testDofCount,
